...
class ChoiceDialogue(override val parent: ScriptProcessor, val choiceDepth: Int, val choicePos: Int, choices: List<String>): DialogueBubble() {
    ...
    var chosen: Int = 0
    val choices = choices.map { DialogueText(it, parent.game, 75) }
    ...
    override fun updateCursorPos(ray: Ray) {
        ...
        choices.forEachIndexed { i, t ->
            ...
            else t.colour = Vec4(1)
        }
    }

    override fun checkHover(ray: Ray, screenPos: Vec2): Boolean {
        return super.checkHover(ray, screenPos).apply { if(this) updateCursorPos(parent.game.camera.getRay(parent.game.input.mouse.lastPos)) }
    }
    ...
    companion object{
        val selectColour = Vec4(107, 86, 100, 255) / 255
    }
}
